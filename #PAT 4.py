...
class Circle: #define class as a circle
    
    __private_var=3.141 #private variable declaration

    def __init__(self,list): #init constructor with parameter as list
        self.list=list
        

    def Areaofcircle(self): #creation of area of circle funtion
        area=[]
        for i in self.list:
            area=area + [i*i]
        return area
    ...
    def ParimeterofCircle(self):#creation od function to calculate parimeter of circle
        parimter=[]
        for i in self.list:
            parimter= parimter + [2*self.__private_var*i]
        return parimter

obj1=Circle([10,501,22,37,100,999,87,351]) #Object of circle class creation
print(obj1.Areaofcircle()) # calling of function to calcualte area of circle
print(obj1.ParimeterofCircle())#Calling of function to calcualte parimter of circle
...
class Tv: #Parent class TV
    ...
    onoff_status=True
    Volume=50
    def __init__(self,brand,channel,volume,onoff_status): #init cosntructor
        self.brand=brand
        self.channel=channel
        self.volume=volume
        self.onoff_status=onoff_status
        
    def set_volume(self): #function to set volumne
    ...
    def set_channel(self): #function to set channel in TV
        if self.channel>50:
            pass
    ...
    def reset_TV(self): #function to reset TV
         self.channel="1"
         self.volume="50"
         return("value set {} channel, {} volumne".format(self.channel,self.volume))
               
    
    def display_status(self): #to show status of TV
         return ("{} at channel {} , volume {}".format(self.brand,self.channel,self.volume))

       


class LED(Tv):#child class in inherit from TV class
